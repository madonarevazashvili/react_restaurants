        ...
        if (item.status !== "Add to cart") {
          ...
          return (
            <div key={key} className='menu_box' style={{height:"320px",backgroundColor:"chocolate", margin: '50px', borderRadius: "10px" }}>
              ...
              <button style={{ color: "black", fontSize: "20px",backgroundColor:"red" }} onClick={() => { if (item.quantity > 1) { item.quantity=item.quantity-1; setPrice(price - item.price); setX(!x);}}}>-</button>
              <button style={{ color: "black", fontSize: "20px", backgroundColor: "red",marginLeft:"140px" }} onClick={() => { if (quantity > 1) { item.quantity=item.quantity+1; setPrice(price + item.price); setX(!x); } }}>+</button>
